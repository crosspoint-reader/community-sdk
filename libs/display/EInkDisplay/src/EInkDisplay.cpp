...
// Non-flashing fast LUT set. Built from the default LUTs by keeping only the
// destination-drive phase. The default LUTs use a 3-phase pattern per 7-byte
// phase block:
//   phase 1: kick to opposite polarity (the visible ghost-clearing flash)
//   phase 2: brief settle
//   phase 3: destination drive (determines final pixel state)
// Initial attempt used phase 1 bytes and produced an inverted image, confirming
// phase 3 is the destination-drive phase. These tables use phase 3's voltage
// pattern in a single phase. Cost: ghosting accumulates because there's no
// inversion sweep, so this is paired with a periodic full refresh.
static constexpr uint8_t MURPHY_LUT_20_FAST[] = {
    0x01, 0x08, 0x08, 0x08, 0x08, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

// LUT_21 (WW) -> drive toward W. Default phase-3 byte was 0x88.
static constexpr uint8_t MURPHY_LUT_21_FAST[] = {
    0x01, 0x88, 0x88, 0x88, 0x88, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

// LUT_22/23 (BW/WB) never fire in the OEM same-buffer scheme, but we mirror the
// destination of the matching solid LUT just in case the controller selects
// them for any transient (old != new) condition during a partial refresh.
static constexpr uint8_t MURPHY_LUT_22_FAST[] = {
    0x01, 0x88, 0x88, 0x88, 0x88, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

static constexpr uint8_t MURPHY_LUT_23_FAST[] = {
    0x01, 0x48, 0x48, 0x48, 0x48, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

// LUT_24 (BB) -> drive toward B. Default phase-3 byte was 0x48.
static constexpr uint8_t MURPHY_LUT_24_FAST[] = {
    0x01, 0x48, 0x48, 0x48, 0x48, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

// Force a full (ghost-clearing) refresh every Nth fast refresh.
#ifndef MURPHY_GHOST_CLEAR_INTERVAL
...
void EInkDisplay::loadMurphyM3FastLut() {
  sendCommand(0x20);
  sendData(MURPHY_LUT_20_FAST, sizeof(MURPHY_LUT_20_FAST));
  sendCommand(0x21);
  sendData(MURPHY_LUT_21_FAST, sizeof(MURPHY_LUT_21_FAST));
  sendCommand(0x22);
  sendData(MURPHY_LUT_22_FAST, sizeof(MURPHY_LUT_22_FAST));
  sendCommand(0x23);
  sendData(MURPHY_LUT_23_FAST, sizeof(MURPHY_LUT_23_FAST));
  sendCommand(0x24);
  sendData(MURPHY_LUT_24_FAST, sizeof(MURPHY_LUT_24_FAST));
}
...
void EInkDisplay::displayBuffer(RefreshMode mode, const bool turnOffScreen) {
  ...
  if (_murphyM3Mode) {
    // Promote FAST to FULL every Nth refresh to clear ghosting. The fast LUTs
    // are destination-only single-phase drives — quick and flash-free, but no
    // inversion sweep means particles don't fully reset between updates.
    bool useFastLut = (mode == FAST_REFRESH);
    if (useFastLut && _murphyM3FastRefreshCount >= MURPHY_GHOST_CLEAR_INTERVAL) {
      useFastLut = false;
      _murphyM3FastRefreshCount = 0;
    }
    if (useFastLut) {
      _murphyM3FastRefreshCount++;
    } else {
      _murphyM3FastRefreshCount = 0;
    }
    if (Serial) {
      ...
      Serial.printf("[%lu]   UC8253_MURPHY_REFRESH requested=%s lut=%s black_pixels=%lu/%lu single_pass=1 lut_each=%d ghost_counter=%u\n",
                    millis(), modeName, useFastLut ? "fast" : "full", blackPixels, bufferSize * 8UL,
                    MURPHY_LOAD_OEM_LUT_EACH_REFRESH, static_cast<unsigned>(_murphyM3FastRefreshCount));
    }
